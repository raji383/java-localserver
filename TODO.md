...
## Core Server
- [ ] Build the non-blocking server loop with `java.nio` `Selector`, one process, one thread.
- [ ] Support listening on multiple ports with one `ServerSocketChannel` per configured port.
